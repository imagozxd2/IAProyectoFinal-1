...
public class Health : MonoBehaviour
{
    ...
    // Nuevo campo serializado para ver en el inspector si la vida es menor a la mitad
    [Header("Health Status")]
    [SerializeField] private bool isHealthBelowHalf;
    ...
    public void UpdateHealthBar()
    {
        ...
        {
            float h = ((float)health / (float)healthMax);
            HealthBarLocal.fillAmount = h;
        }

        // Actualizamos el bool según el estado de la vida
        isHealthBelowHalf = (health < (healthMax / 2));
    }

    public virtual void LoadComponent()
    {
        health = healthMax;
        isHealthBelowHalf = false; // Inicializamos el estado al comenzar
    }
}
